...
public class SASTScanner_1 implements ISASTConstants, IJSONConstants {
    ...
    private static final File BAMBOO_INSTALL_DIR = new File(System.getProperty("user.dir"),".appscanBamboo");
	...
	public SASTScanner_1(LogHelper logger, ProcessService processService) {
		this.logger = logger;
    ...
    private Map<String, String> getScanProperties(String appId, String target, String scanName) {
        Map<String, String> properties = new HashMap<String, String>();
        properties.put(CoreConstants.TARGET, target);
        properties.put(CoreConstants.SCANNER_TYPE, "SAST");
        ...
        properties.put("APPSCAN_IRGEN_CLIENT", "bamboo");
        properties.put("APPSCAN_CLIENT_VERSION", "1");
        properties.put("IRGEN_CLIENT_PLUGIN_VERSION", "1");
    ...
    private void setInstallDir() {
        if (SystemUtil.isWindows() && System.getProperty("user.home").toLowerCase().indexOf("system32")>=0) {
            System.setProperty(CoreConstants.SACLIENT_INSTALL_DIR, BAMBOO_INSTALL_DIR.getPath());
        }
    }

    public void scheduleDynamicScan(TaskContext taskContext) throws TaskException {
        logger.info("Schedule Dynamic Scan");
        String appId = taskContext.getConfigurationMap().get(CFG_APP_ID);
        IProgress progress = new ScanProgress(logger);
        BambooAuthenticationProvider authenticationProvider = new BambooAuthenticationProvider(username, password);
        DASTScanFactory scanFactory = new DASTScanFactory();

        loginToASoC(taskContext);                       // TODO
        String target = "https://demo.testfire.net/";   // TODO
//        String target = "httfdvspd sbsd:bs/db/dsbdsdebmdsbods.bdtesbsdtbsdfbidsbre.netbsdb/bdsbs";   // TODO

	    Map<String, String> scanProperties = getScanProperties(appId, target, taskContext.getBuildContext().getPlanName());
	    IScan scan = scanFactory.create(scanProperties, progress, authenticationProvider);
        try {
            scan.run();
            logger.info("Scan scheduled Successfully");
            jobId = scan != null ? scan.getScanId() : "";
            logger.info("Scan Id: ", jobId);
        } catch (Exception e) {
            logger.error("Exception in Scheduling Scan", e.getLocalizedMessage());
...
//		loginToASoCSDK(taskContext);
		IProgress progress = new ScanProgress(logger);
		BambooAuthenticationProvider authenticationProvider = new BambooAuthenticationProvider(username, password);
		SASTScanFactory scanFactory = new SASTScanFactory();
		String target = workingDir.toString() + File.separator + irxBaseName + ".irx";
		IScan scan = scanFactory.create(getScanProperties(appId, target, irxBaseName), progress, authenticationProvider);
		try {
			setInstallDir();
			scan.run();
            logger.info("Scan scheduled Successfully");
